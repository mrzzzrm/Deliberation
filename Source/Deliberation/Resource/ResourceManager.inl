...
void ResourceManager::setLoader(const ResourceLoader <ResourceType> &loader) {
    m_resourceContainersByTypeID[TypeID::value<ResourceManager, ResourceType>()] =
        std::make_shared<ResourceContainer < ResourceType>>
    (loader);
}
...
ResourceManager::ResourceContainer<ResourceType> &ResourceManager::containerRef() {
    ...
    AssertM(iter != m_resourceContainersByTypeID.end(),
            std::string("Couldn't find container for '") + typeid(ResourceType).name() + "'");
    return *std::static_pointer_cast<ResourceContainer < ResourceType>>
    (iter->second);
}
