...
CuboidMesh::CuboidMesh(const glm::vec3 & size) : m_size(size) {}
...
Mesh CuboidMesh::generate() const
{
    ...
    std::vector<Mesh::Face> faces({{{0, 4, 7, 1}}, // l
                                   {{6, 5, 3, 2}}, // r
                                   {{0, 1, 2, 3}}, // b
                                   {{4, 5, 6, 7}}, // r
                                   {{1, 7, 6, 2}}, // b
    ...
    return Mesh(std::move(vertices), std::move(face_data), std::move(faces));
}
}