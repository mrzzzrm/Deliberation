...
Camera2D::Camera2D()
    : m_scale(1.0f), m_center(0.0f, 0.0f), m_aspectRatio(1.0f), m_dirty(true)
{
}
...
float Camera2D::scale() const { return m_scale; }

const glm::vec2 & Camera2D::center() const { return m_center; }

float Camera2D::aspectRatio() const { return m_aspectRatio; }

void Camera2D::setScale(float scale) { m_scale = scale; }
...
const glm::quat & Camera2D::orientation() const { return m_orientation; }
...
void Camera2D::update() const
{
    ...
    m_projection = glm::scale(
        glm::mat4(1.0f), glm::vec3(m_scale / m_aspectRatio, m_scale, 1.0f));
