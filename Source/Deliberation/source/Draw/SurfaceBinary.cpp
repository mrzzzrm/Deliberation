...
SurfaceBinary::SurfaceBinary(unsigned int width,
                             unsigned int height,
                             PixelFormat format):
    m_width(width),
    m_height(height),
...
SurfaceBinary::SurfaceBinary(Blob && data,
    ...
    m_format(format)
{
    Assert(expectedDataSize() == m_data.size(),
           "Requested size: " + std::to_string(width) + "x" + std::to_string(height) + "x" + std::to_string(m_format.bytesPerPixel()) + "; "
...
{
    return std::string("SurfaceBinary {") + std::to_string(m_width) + "x" + std::to_string(m_height) + ": " + m_format.toString() + "}";
}
