...
PhysicsWorldSystem::PhysicsWorldSystem(World &world, PhysicsWorld &physicsWorld)
    : Base(world, ComponentFilter::requires<RigidBodyComponent>()), m_physicsWorld(physicsWorld) {
}
...
void PhysicsWorldSystem::onEntityPrePhysicsUpdate(Entity &entity, float physicsTimestep) {
    if (entity.hasComponent<Transform3DComponent>()) {
        entity.component<RigidBodyComponent>().value()->transform() =
            entity.component<Transform3DComponent>().value();
    }
