...
{
    static std::unordered_map<TypeID::value_t, std::vector<TypeID::value_t>>
        subscriptionsByComponentType;
};

template<typename ComponentType, typename... Subscriptions>
class ComponentSubscriptions : public ComponentSubscriptionsBase
{
  public:
  ...
  private:
    std::unordered_map<
        TypeID::value_t,
        std::function<void(ComponentBase & componentBase, const void * event)>>
        m_dispatcherByEvent;
};
}
