                       ...
                       std::vector<glm::vec2> & result)
{
    ...
    {
        auto edge0 = glm::vec2(clipping[1].x - clipping[0].x, clipping[1].y - clipping[0].y);
        auto edge1 = glm::vec2(clipping[2].x - clipping[1].x, clipping[2].y - clipping[1].y);
...
//    std::cout << std::endl;
    ...
    for (int c0 = 0; c0 < clipping.size(); c0++)
    {
        ...
        Ray2D edge = clippingCCW ? Ray2D::fromTo(C0, C1) : Ray2D::fromTo(C1, C0);
...
//        std::cout << " -> ";
        ...
        for (int i = 0; i < input.size(); i++)
        {
           ...
           if (PointRay2DHalfspace(E, edge) <= 0)
           {
               ...
               {
                   bool exists;
                   output.push_back(Ray2DIntersectionPoint(Ray2D::fromTo(S, E), edge, exists));
                   Assert(exists, "");
               }
               output.push_back(E);
           }
