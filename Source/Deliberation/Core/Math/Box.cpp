...
Box::Box(
    ...
    const glm::vec3 & y,
    const glm::vec3 & z)
    : m_p(p), m_axes(glm::normalize(x), glm::normalize(y), glm::normalize(z))
{
...
Box::Box(const glm::vec3 & halfExtent, const Transform3D & transform)
{
    ...
    auto x =
        transform.directionLocalToWorld(glm::vec3(halfExtent.x, 0.0f, 0.0f));
    auto y =
        transform.directionLocalToWorld(glm::vec3(0.0f, halfExtent.y, 0.0f));
    auto z =
        transform.directionLocalToWorld(glm::vec3(0.0f, 0.0f, halfExtent.z));
...
const glm::vec3 & Box::p() const { return m_p; }

const glm::vec3 & Box::x() const { return m_axes[0]; }

const glm::vec3 & Box::y() const { return m_axes[1]; }

const glm::vec3 & Box::z() const { return m_axes[2]; }

const glm::vec3 & Box::halfExtent() const { return m_halfExtent; }

const glm::mat3 & Box::orientationMatrix() const { return m_axes; }
