...
const SurfaceBinary & SurfaceDownloadImpl::result() const
{
    ...
    if (!isDone())
    {
        while (true)
        {
            auto r = gl::glClientWaitSync(
                sync, gl::GL_SYNC_FLUSH_COMMANDS_BIT, 1000 * 1000u);
            if (r == gl::GL_WAIT_FAILED)
            {
    ...
    switch (type)
    {
    case gl::GL_BYTE: fillSurfaceBinary<gl::GLbyte>(numValues); break;
    case gl::GL_UNSIGNED_BYTE: fillSurfaceBinary<gl::GLubyte>(numValues); break;
    case gl::GL_SHORT: fillSurfaceBinary<gl::GLshort>(numValues); break;
    case gl::GL_UNSIGNED_SHORT:
        fillSurfaceBinary<gl::GLushort>(numValues);
        break;
    case gl::GL_INT: fillSurfaceBinary<gl::GLint>(numValues); break;
    case gl::GL_UNSIGNED_INT: fillSurfaceBinary<gl::GLuint>(numValues); break;
    case gl::GL_FLOAT: fillSurfaceBinary<gl::GLfloat>(numValues); break;
...
void SurfaceDownloadImpl::fillSurfaceBinary(std::size_t numValues) const
{
    ...
    surfaceBinary.reset(
        std::move(data), surface.width(), surface.height(), surface.format());
}
}
