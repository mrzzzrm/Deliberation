...
std::string GLGetActiveUniformBlockName(gl::GLuint glProgramName, gl::GLuint blockIndex)
{
    gl::GLint length;
    glGetActiveUniformBlockiv(glProgramName, blockIndex, gl::GL_UNIFORM_BLOCK_NAME_LENGTH, &length);
    Assert(length > 1, ""); // Has to include at least 1 char and '\0'

    std::vector<char> name(length);
    gl::glGetActiveUniformBlockName(glProgramName, blockIndex, length, nullptr, name.data());
...
gl::GLint GLGetActiveUniformBlockInt(gl::GLuint glProgramName, gl::GLuint blockIndex, gl::GLenum pname)
{
    gl::GLint value = 0;
