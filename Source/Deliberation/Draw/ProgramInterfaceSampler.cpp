...
SamplerType ProgramInterfaceSampler::type() const
{
    return m_type;
}

Type ProgramInterfaceSampler::valueType() const
{
    return m_valueType;
}
...
u32 ProgramInterfaceSampler::size() const
{
    return m_size;
}
...
std::string ProgramInterfaceSampler::toString() const
{
    ...
    stream << (gl::GLenum)m_type << " <" << m_name << "> (Size: " << m_size << "; Location: " << m_location << ")";
    return stream.str();
}
