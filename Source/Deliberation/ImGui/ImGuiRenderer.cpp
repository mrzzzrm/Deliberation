    ...
    : SingleNodeRenderer(renderManager, RenderPhase::Overlay, "ImGui")
{
    ...
    io.Fonts->GetTexDataAsRGBA32(
        &m_fontAtlasPixels, &m_fontAtlasWidth, &m_fontAtlasHeight);
}
...
void ImGuiRenderer::onSetupRender()
{
    ...
    const auto textureBinary =
        TextureLoader(
            Blob::fromRawData(
                m_fontAtlasPixels, m_fontAtlasWidth * m_fontAtlasHeight * 4),
            m_fontAtlasWidth,
            m_fontAtlasHeight,
...
void ImGuiRenderer::render()
{
    ...
    for (int l = 0; l < drawData->CmdListsCount; l++)
    {
        ...
        m_vertexBuffer.rawUpload(
            Blob::fromRawData(vertexData, vertexDataSize),
            cmdList->VtxBuffer.Size);
        m_indexBuffer.rawUpload(
            Blob::fromRawData(indexData, indexDataSize),
            cmdList->IdxBuffer.Size);

        //   std::cout << "  " << l << ": Cmds: " << cmdList->CmdBuffer.Size <<
        //   std::endl;
        for (int c = 0; c < cmdList->CmdBuffer.Size; c++)
        {
            const auto & drawCmd = cmdList->CmdBuffer[c];

            //            std::cout << drawCmd.ElemCount << ": Ignoring clip
            //            rect: " << drawCmd.ClipRect.x << " " <<
            //            drawCmd.ClipRect.y << " " <<
            //                 drawCmd.ClipRect.z << " " << drawCmd.ClipRect.w
            //                 << " Texture: " << drawCmd.TextureId <<
            //                 std::endl;

            const auto & clipRect = drawCmd.ClipRect;

            m_draw.state().rasterizerState().enableScissorRect(
                (u32)clipRect.x,
                (u32)(io.DisplaySize.y - clipRect.w),
                (u32)(clipRect.z - clipRect.x),
                (u32)(clipRect.w - clipRect.y));

            m_draw.setIndexBufferRange(
                m_indexBuffer, indexOffset, drawCmd.ElemCount);
